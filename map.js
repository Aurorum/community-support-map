...
function setDroppingLocationPin() {
	if ( sessionStorage.getItem( 'isDroppingLocationPin' ) === 'true' ) {
		...
		document.getElementById( 'save-location-instructions' ).style.display = 'block';
		document.getElementById( 'add-location-button' ).innerHTML = 'Add your location';
	} else {
...
function interpretApiResponse( arr ) {
	...
	for ( i = 0; i < arr.values.length; i++ ) {
		...
		new mapboxgl.Marker( el ).setLngLat( apiCoordinates ).setPopup( popup ).addTo( map );
	}
}
...
map.on( 'click', function ( e ) {
	if ( sessionStorage.getItem( 'isDroppingLocationPin' ) === 'true' ) {
		...
		document.getElementById( 'location-saver-prompt' ).style.display = 'none';
		document.getElementById( 'save-location-instructions' ).style.display = 'none';
		document.getElementById( 'add-location-button' ).classList.remove( 'is-scary' );
		document.body.classList.remove( 'is-placing-marker' );
		document.getElementById( 'add-location-button' ).innerHTML = 'Change location';
	}
	sessionStorage.setItem( 'isDroppingLocationPin', 'false' );
} );
...
function apiInterpretation( arr ) {
	var apiResponse = arr.values[ 0 ] + '';
	var apiResponseData = apiResponse.split( /,+/ );
}
...
function fireInformation() {
	if (
		...
		document.getElementById( 'save-location-form' ).classList.add( 'submission-sent' );
	}
}
